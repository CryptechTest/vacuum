...
minetest.register_abm({
	...
	action = vacuum.throttle(1250, function(pos)
		...
		if (vacuum.is_pos_in_space(pos)) then
			--if vacuum.has_in_range(pos, "vacuum:vacuum", 2, 10) and vacuum.near_powered_airpump(pos, 3) then
			--	minetest.set_node(pos, {name = "vacuum:atmos_thin"})
			--elseif vacuum.has_in_range(pos, "vacuum:vacuum", 2, 100) and vacuum.near_powered_airpump(pos, 4) then
			--	minetest.set_node(pos, {name = "vacuum:atmos_thin"})
			--elseif vacuum.has_in_range(pos, "vacuum:vacuum", 2, 25) and vacuum.near_powered_airpump(pos, 8) then
				--minetest.set_node(pos, {name = "vacuum:atmos_thin"})
		...
		if vacuum.is_pos_in_space(pos) then
			if vacuum.near_powered_airpump(pos, 8) then --12
				if vacuum.has_in_range(pos, "vacuum:atmos_thick", 1, 5) then
					minetest.set_node(pos, {name = "vacuum:atmos_thick"})
				elseif vacuum.has_in_range(pos, "vacuum:atmos_thin", 1, 1) and not vacuum.has_in_range(pos, "vacuum:vacuum", 1, 9) then
					minetest.set_node(pos, {name = "vacuum:atmos_thick"})
				end
			end
		end
...
minetest.register_abm({
	...
	action = vacuum.throttle(2500, function(pos)
		...
		if vacuum.is_pos_in_space(pos) then
			...
			elseif vacuum.has_in_range(pos, "vacuum:vacuum", 1, 10) and vacuum.near_powered_airpump(pos, 10) then
				minetest.set_node(pos, {name = "vacuum:vacuum"})
